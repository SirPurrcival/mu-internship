...
delta_f_range_net1 = [(0.9, 17), (0.9, 18), (0.9, 19), (0.9, 20), (1.0, 16), (1.0, 17), (1.0, 18),
                      (1.0, 19), (1.1, 15), (1.1, 16), (1.1, 17), (1.2, 14), (1.2, 15)]

# delta_f_range_net2 = [(1.0, 18), (1.0, 16), (0.9, 19), (0.9, 18), (0.9, 17), (0.9, 16), (0.8, 18)]

ck_range = np.arange(0.0, 0.11, 0.01)

deep_input_range = np.arange(0, 701, 600)

# total_iter = len(deep_input_range) * ((len(delta_f_range_net1) * len(ck_range)) + (len(delta_f_range_net2) * len(ck_range)))
total_iter = len(deep_input_range) * (len(delta_f_range_net1) * len(ck_range))
...
for deep_input in deep_input_range:
    ...
    for delta_f in delta_f_range_net1:
        for ck in ck_range:
            ...
            print("Starting step...")
            st = time.time()
            params['cell_params_net1'][0]['tau_syn_ex']  = delta_f[0]
            params['cell_params_net1'][0]['tau_m']       = delta_f[1]
            params['intercircuit_strength']              = ck
            
            params['second_net'] = True
            
            params['deep_input'] = deep_input
            ...
            opt_results['net2_CV'].append(data['net2_CV'])
            opt_results['net2_mean'].append(data['net2_ISI_mean'])
            opt_results['net2_stdev'].append(data['net2_ISI_std'])
            opt_results['net2_firing_rate'].append(data['net2_firing_rate'])
    ...
    #         print("Starting step...")
    #         st = time.time()
    #         params['cell_params_net2'][0]['tau_syn_ex']  = delta_f[0]
    #         params['cell_params_net2'][0]['tau_m']       = delta_f[1]
    #         params['intercircuit_strength']              = ck
    ...
    delta_f_data = [-12, -10, -8, -6, -4, -2, 0, 2, 4, 6, 8, 10, 12]
    ...
    net1_fr1_data   = [x[0] if type(x) == np.ndarray else x for x in opt_results['net1_firing_rate']]
    net1_fr2_data   = [x[2] if type(x) == np.ndarray else x for x in opt_results['net1_firing_rate']]
    net2_fr1_data   = [x[0] if type(x) == np.ndarray else x for x in opt_results['net2_firing_rate']]
    net2_fr2_data   = [x[2] if type(x) == np.ndarray else x for x in opt_results['net2_firing_rate']]
    
    ## Plot heatmaps
    plot_heatmap(delta_f_data, ck_data, plv_inter_data, f"PLV intercircuit\nDeep: {deep_input}", round_to = 1)
    plot_heatmap(delta_f_data, ck_data, plv_intra1_data, f"PLV - intracircuit Net 1\nDeep: {deep_input}", round_to = 1)
    plot_heatmap(delta_f_data, ck_data, plv_intra2_data, f"PLV - intracircuit Net 2\nDeep: {deep_input}", round_to = 1)
    plot_heatmap(delta_f_data, ck_data, net1_fr1_data, f"Upper layer FR - Net 1\nDeep: {deep_input}", round_to = 0)
    plot_heatmap(delta_f_data, ck_data, net1_fr2_data, f"Lower layer FR - Net 1\nDeep: {deep_input}", round_to = 0)
    plot_heatmap(delta_f_data, ck_data, net2_fr1_data, f"Upper layer FR - Net 2\nDeep: {deep_input}", round_to = 0)
    plot_heatmap(delta_f_data, ck_data, net2_fr2_data, f"Lower layer FR - Net 2\nDeep: {deep_input}", round_to = 0)
