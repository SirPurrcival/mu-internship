...
# setup neurons
E_lif = nest.Create("glif_cond", NE,
                    params={"spike_dependent_threshold": False,
                            "after_spike_currents":      True,
                            "adapting_threshold":        False})
I_lif = nest.Create("glif_cond", NI,
                    params={"spike_dependent_threshold": False,
                            "after_spike_currents":      True,
                            "adapting_threshold":        False})
...
tmp = nest.GetStatus(E_lif)

nya = nest.GetStatus(mm)
tmp = [x for x in nya[0]["events"]["ASCurrents_sum"] if x != 0]
tmp2 = nya[0]["events"]["ASCurrents_sum"].shape
mmlist = []
for d in mm:
    mmlist.append(nest.GetStatus(d)[0]["events"])

approximate_lfp(resolution, mmlist, 1000)
